...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/version/doaddversion.html?id="+obj.attr("appinfoid");
});
...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/version/modify.html?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/appinfo/modify.html?id="+ obj.attr("appinfoid");
	}else{
